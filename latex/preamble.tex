%% pdf engine: xelatex
\usepackage[paper=a4paper, left=1.0in, right=1.0in, top=1.0in, bottom=0.6in, footskip=1.5cm, includefoot, heightrounded]{geometry}
\usepackage{booktabs}
\usepackage{xcolor}
...
% table typesetting
% needspace lets a block ask for a minimum amount of room: if less than that is
% left on the page, LaTeX breaks first. Pandoc renders markdown pipe tables as
% `longtable`, which is built to split across pages, so `\needspace` before a
% table is the way to keep one whole without rewriting it as raw LaTeX.
