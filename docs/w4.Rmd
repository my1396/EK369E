...
Now we allow for short selling.
...
Computing the global minimum variable portfolio is a constriained optimization problem:
...
- If  $\rho_{AB}=-1$, there exists a portfolio of A and B that has positive expected return and zero variance.
...
Tangency portfolio represents the portfolio which maximizes Sharpe ratio. 
...
- If short sales are not allowed, then set `shorts = TRUE`.


You can compute the portfolio with a target expected return.
```{r}
target.return <-  mu.vec[1]
e.port.msft <-  efficient.portfolio(mu.vec, sigma2.mat, target.return)
e.port.msft
```
...
Use an annual risk-free rate of $3\%$ per year for the T-bill. 
...
1. Create the following portfolios.
    ...
    - Combinations of Boeing and T-bills (with $x{boeing}=0,0.1,\ldots,2$)
    
    - Combinations of Microsoft and T-bills (with $x{msft}=0,0.1,\ldots,2$)
...
2. Compute the global minimum variance portfolio (GMV).
    ...
    - Compute the weights of Boeing and Microsoft in the GMV portfolio.
...
3. Compute the tangency portfolio.
    ...
    - Compute the weights of Boeing and Microsoft in the tangency portfolio.
