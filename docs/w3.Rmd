...
Now we build an equal-weighted portfolio consisting of 10 stocks per period, rebalanced annually. Then compute and plot portfolio returns using `PerformanceAnalytics`.


```{r}
# Define list of selected ISINs for each year (2015 to 2022, 8 yrs)
...
`table.SFM` creates a table of **CAPM-Related Measures**. SFM stands for Single Factor Model.

```{r}
with(ptf_ret3[, c("Date", "port_ret", "mkt_return_monthly", "rf_1month")] %>%
    mutate(Date = ymd(Date)) %>% 
    xts(x=.[,-1], order.by=.[[1]]),
    table.SFM(
        Ra = port_ret,
...
- `Tracking Error`: the unexplained portion of the investment's performance relative to a benchmark, defined as the standard 
