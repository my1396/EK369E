...
$$ where $r_i$ is the return of asset $i$; $r_f$ is the riskless rate of interest; $r_m$ is the return of the market portfolio.
...
-   $\alpha_i>0$ means positive abnormal return, this could be caused, for example, by positive estimate of the security's prospect, or you think the security is underpriced, and given time, the price will converge to its equilibrium level and therefore offers an attractive expected return.
...
We use monthly data of *Archer* over the period 2015-01 to 2020-12.
...
-   The slope ($2.234$) is the estimate of beta, meaning that its share price tended to move 2.234% for every 1% move in the market index.
    ...
    -   The standard deviation of the residual is $20.4\%$ per month, or \`20.4%\*sqrt(12)=70.67%\`. This stands for firm-specific risk.
...
-   Notice that when $r_f$ is constant, the$$
    ...
    &nbsp; &nbsp; &nbsp; &nbsp; $R^e_i=r_i-r_f$ stands for excess return on asset $i$, and

    &nbsp; &nbsp; &nbsp; &nbsp; $R^e_m=r_m-r_f$ stands for excess return on the market index.
...
If CAPM holds, $\gamma_0$ should be equal to zero, and $\gamma_1$ should be equal to the mean excess return on the market.
...
-   Solution:
    ...
    Measure $\beta$'s based on portfolios will be less affected by the measurement error than individual $\beta_i$ due to aggregation.

Here we demonstrate an example using individual $\beta_i$.
...
-   Construct equally-weighted decile portfolios based on the ranking of $\beta_i$, do the same analysis based on portfolio $\beta_p$ and see if there exists an SML.
