...
```{r tmp, out.width="50%", fig.cap="EMH", echo=FALSE}
# All defaults
...
The estimate is not statistically significantly different from $1$. Therefore, we fail to reject the null hypothesis of random walk. Equivalently, we say that this provides little evidence against the random walk hypothesis.
...
<div class = "boxed">
```{exercise proof4}
To show that $$\text{Cov}(e_t, e_{t-k}) = 0$$ is equivalent to show that $$\text{Cov}(r_t, r_{t-k}) = 0.$$
...
$r_t$ is the first-difference of price at time $t$.
...
That is, the covariance of the first-differences equals to the covariance of the increments. 
...
@Ljung1979 provided a correction which yields a better for to the $\chi_m^2$ for small sample sizes:
...
Lo and MacKinlay present a number of test statistics based on the VR(q). Check Lo and MacKinlay [-@Lo-Mackinlay1988; -@Lo1989] for example.
