...
- **Arithmetic mean** (AM) portfolio return
  $$\begin{aligned}
  \text{Arithmetic } \bar{r}_{p,\, 0:T} = \bar{r}_A &= \frac{1}{T}\sum_{t=1}^T r_{p,t} \\
  &= \frac{1}{T} \left(r_{p,1} + r_{p,2} + \cdots + r_{p,T}\right)
  \end{aligned}$$

- **Geometric mean** (GM) portfolio return
  $$\begin{aligned}
  \text{Geometric } \bar{r}_{p,\, 0:T} = \bar{r}_G &= \left[\prod_{t=1}^T (1+r_{p,t}) \right] ^{\frac{1}{T}}-1 \\
  &= \sqrt[T]{(1 + r_{p,1}) \times (1 + r_{p,2}) \times \cdots \times (1 + r_{p,T})} - 1
  \end{aligned}$$

- Relationship between AM and GM:
  $$\bar{r}_G \approx \bar{r}_A - 0.5\sigma^2$$
...
- When there is variation, the geometric average is always smaller than the arithmetic average, $\bar{r}_G \leq \bar{r}_A.$ Higher variance in returns leads to a larger difference between the two averages.
...
(c) The geometric return is $$\begin{aligned} \bar{r}_G &= [(1+r_1)(1+r_2)]^{1/2} - 1 \\ &= [(1 + 0.10) * (1 - 0.10)]^{1/2} - 1 \\ &≈ -0.50\% \end{aligned}$$
...
Where: $r_t$ = return in each period; $\bar{r}$ = *arithmetic* average return; $T$ = number of periods.
...
- **Portfolio Return** $\left( E(r_P) \right)$:
  $$E(r_P) = w_1 \cdot E(r_1) + w_2 E(r_2)$$
...
- $\sigma_{1,2}$ = covariance between assets 1 and 2, where $\sigma_{1,2}=\rho_{1,2}\sigma_1\sigma_2$; 
